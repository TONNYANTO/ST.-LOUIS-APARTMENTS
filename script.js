...
// ✅ FIX: Application State — NO localStorage. These are kept in memory and
//         always synced from Firebase via real-time listeners below.
let tenants = [];
let payments = [];
let currentTheme = localStorage.getItem("theme") || "light"; // theme is fine in localStorage
...
// ✅ FIX: Initialize Application — set up Firebase real-time listeners FIRST,
//         then render UI only after data has loaded from Firebase.
document.addEventListener("DOMContentLoaded", () => {
  initializeTheme();
  initializeYearSelects();
  populateRoomSelects();
  loadIncomeFromFirebase();

  // ✅ Real-time listener for tenants — fires on ALL devices instantly
  db.collection("tenants").onSnapshot((snapshot) => {
    tenants = snapshot.docs.map((doc) => ({ ...doc.data(), firestoreId: doc.id }));
    updateTenantTables();
    updateDashboard();
    updateRoomStatus();
  }, (error) => {
  ...
  // ✅ Real-time listener for payments — fires on ALL devices instantly
  db.collection("payments").onSnapshot((snapshot) => {
    payments = snapshot.docs.map((doc) => ({ ...doc.data(), firestoreId: doc.id }));
    updatePaymentTable();
    updateDashboard();
  }, (error) => {
...
// ✅ Load income totals from Firebase
function loadIncomeFromFirebase() {
  db.collection("statistics").doc("incomeTotals").onSnapshot((doc) => {
    if (doc.exists) {
      const data = doc.data();
      if (data.monthlyIncome !== undefined) {
        document.getElementById("monthlyIncome").textContent = `KSh ${data.monthlyIncome.toLocaleString()}`;
      }
      if (data.yearlyIncome !== undefined) {
        document.getElementById("yearlyIncome").textContent = `KSh ${data.yearlyIncome.toLocaleString()}`;
      }
    }
  });
}
...
document.getElementById("tenantForm").addEventListener("submit", (e) => {
  ...
  // ✅ FIX: Save ONLY to Firestore — the real-time listener above will
  //         automatically update the UI on ALL devices.
  db.collection("tenants")
    ...
    .then(() => {
      ...
      // ❌ REMOVED: tenants.push(tenant) and localStorage.setItem(...)
      //    The onSnapshot listener handles updating tenants[] and the UI.
    })
    .catch((error) => {
...
function createTenantRow(tenant) {
  ...
  // ✅ FIX: Use firestoreId (the Firestore document ID) for removal
  row.innerHTML = `
        ...
        <td>
            <div class="action-buttons">
                <button class="btn-danger" onclick="removeTenant('${tenant.firestoreId}')" title="Remove Tenant">
                    Remove
                </button>
            </div>
        </td>
...
// ✅ FIX: removeTenant now deletes from Firestore (not localStorage)
function removeTenant(firestoreId) {
  if (confirm("Are you sure you want to remove this tenant?")) {
    db.collection("tenants")
      .doc(firestoreId)
      .delete()
      .then(() => {
        console.log("✅ Tenant removed from Firebase");
        // The onSnapshot listener will automatically update tenants[] and the UI
      })
      .catch((error) => {
        console.error("❌ Error removing tenant:", error);
        alert("Failed to remove tenant. Check console for details.");
      });
  }
}
...
document.getElementById("paymentForm").addEventListener("submit", (e) => {
  ...
  const existingPayment = payments.find(
    (p) => p.roomNumber === roomNumber && p.month === month && p.year === year
  );
  ...
  if (existingPayment) {
    ...
    // ✅ FIX: Delete the old payment from Firestore before adding updated one
    db.collection("payments").doc(existingPayment.firestoreId).delete();
  }
  ...
  // ✅ FIX: Save ONLY to Firestore — onSnapshot listener updates payments[] and UI on all devices
  db.collection("payments")
    .add(payment)
    .then(() => {
      console.log("✅ Payment saved to Firebase");
      // ❌ REMOVED: payments.push(payment) and localStorage.setItem(...)
      document.getElementById("paymentForm").reset();

      const statusMessage = isFullyPaid
        ? `Payment recorded successfully! Rent is now FULLY PAID.`
        : `Payment recorded successfully! Remaining balance: KSh ${Math.max(0, finalBalance).toLocaleString()}`;
      alert(statusMessage);
    ...
    .catch((error) => {
      ...
      alert("Failed to save payment to Firebase. Check console for details.");
    });
});
...
function updateDashboard() {
  ...
  const monthlyPayments = payments.filter(
    (payment) =>
      payment.month === monthNames[currentMonth] &&
      Number.parseInt(payment.year) === currentYear
  );
  const monthlyIncome = monthlyPayments.reduce((sum, payment) => sum + payment.amountPaid, 0);

  const yearlyPayments = payments.filter(
    (payment) => Number.parseInt(payment.year) === currentYear
  );
...
// Edit Income (saves to Firestore)
